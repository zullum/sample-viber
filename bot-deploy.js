...
function deployToHeroku(accessToken) {
  if (!shell.which("heroku")) {
    ...
    if (
      shell.exec(
        "heroku config:set VIBER_PUBLIC_ACCOUNT_ACCESS_TOKEN_KEY='" +
          accessToken +
          "'"
      ).code !== 0
    ) {
...
function deployToNow(accessToken) {
  if (!shell.which("now")) {
    shell.echo(
      colors.red.bold(
        "Sorry, this script requires now CLI. Please follow the installment procedure at https://now.sh"
      )
    );
    shell.exit(0);
  }
  ...
  if (
    shell.exec(
      "now -e VIBER_PUBLIC_ACCOUNT_ACCESS_TOKEN_KEY='" + accessToken + "'"
    ).code !== 0
  ) {
    ...
    let deploymentMethodSchema = {
      properties: {
        option: {
          pattern: /^(1|2)$/,
          description:
            "How would you like to deploy the script?\n" +
            colors.green("1") +
            " for https://heroku.com\n" +
            colors.green("2") +
            " for https://now.sh\n",
